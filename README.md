...
- 주문 입력
  - 조건
    - 대괄호와 -로 구분한다.(예 : [ㅁㅁ-3],[ㅂㅂ-1])
  - 예외
    - 수량이 부족할 경우
    - 해당 제품이 존재하지 않을 경우
    - 공백이 포함된 경우
- 프로모션
  - 프로모션이 적용되지 않은 상품에 대한 안내에서 "N"을 입력 받으면 주문을 취소한다.
  - 프로모션 적용으로 증정품 추가에 대한 안내에서 "N"을 입력 받으면 멤버십을 적용한다. 
  - 오늘 날짜가 프로모션 기간에 해당하지 않으면 안내문을 출력하고 정가로 결제한다.
- 보유하고 있는 상품
  - 프로모션 상품이 존재할 경우, 보유하고 있는 상품을 출력할 때 프로모션이 적용되는 상품과 적용되지 않는 상품을 둘 다 출력한다.
...
- **3주 차 공통 피드백**을 최대한 반영한다.
  - main() 함수를 포함한 모든 함수는 10 라인 이하로 제한한다.
  - 비즈니스 로직과 UI 로직을 분리한다.
  - 연관성 있는 상수는 enum으로 그룹화한다.
  - final 키워드로 값의 변경을 막는다.
  - private으로 외부로부터 객체의 상태 접근을 제한한다.
  - 객체는 get 대신 내부에서 스스로 데이터를 처리하도록 메시지를 던지게 한다.
  - 인스턴스 변수의 수를 최소화하여 객체의 복잡도를 낮춘다.
...
- 구현에 필요한 상품 목록과 행사 목록을 파일 입출력을 통해 불러온다.
  ...
  - 두 파일 모두 내용의 형식을 유지단다면 값은 수정할 수 있다.
...
---
## 🐌 과제 진행 요구 사항
- [woowacourse-precourse/java-convenience-store-7](https://github.com/woowacourse-precourse/java-convenience-store-7) 저장소를 복제한다(use this template).
- 생성한 비공개 저장소 [33jyu33/java-convenience-store-7-33jyu33](https://github.com/33jyu33/java-convenience-store-7-33jyu33)를 clone한다.
...
### 미션 제출 방법
- 미션 구현을 완료한 후 GitHub을 통해 제출해야 한다._[(프리코스 과제 제출 문서 참고)](https://docs.google.com/document/d/1cmg0VpPkuvdaetxwp4hnyyFC_G-1f2Gr8nIDYIWcKC8/edit?tab=t.0#heading=h.1x8npe961lrb)_
